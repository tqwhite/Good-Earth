...
class UserController extends Q_Controller_Base {
	...
	public function resetpwAction() {
		error_log("PW RESET EXECUTION START: identifier: {$inData['identifier']} [UserController.php]");
		...
		if (!$user) {
			$this->_helper->json(array(status => - 1, messages => array(array('identifier', "No user with that identifier found")), data => array()));
		} else {
			...
			error_log("PW RESET EXECUTION COMPLETE: identifier: {$inData['identifier']} [UserController.php]");
	...
	private function sendResetEmail($userObj) {
		error_log("PW RESET EMAIL START: email server: user: {$userObj->userName}, dest email: {$userObj->emailAdr} [UserController.php]");
	...
	public function setpwAction() {
		...
		if (count($errorList) > 0) {
			...
			$this->_helper->json(array(status => $status, messages => $messages, data => array()));
	...
	public function searchAction() {
		...
		$searchFragment = $inData['data']['searchFragment'];
		$userObj = new \Application_Model_User();
		$userList = $userObj->searchByUserName($searchFragment);

		$this->_helper->json(array(status => 1, messages => $messages, data => $userList));
